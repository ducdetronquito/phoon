...
suite "Endpoints":
    ...
    test "DELETE endpoint":
        var ctx = Context.new(Request(HttpMethod.HttpDelete, "https://yumad.bro/"), Config())
        var app = App.new()
        app.delete("/",
    ...
    test "GET endpoint":
        var ctx = Context.new(GetRequest("https://yumad.bro/"), Config())
        var app = App.new()
        app.get("/",
            proc (ctx: Context) {.async.} =
                discard
    ...
    test "HEAD endpoint":
        var ctx = Context.new(Request(HttpMethod.HttpHead, "https://yumad.bro/"), Config())
        var app = App.new()
        app.head("/",
            proc (ctx: Context) {.async.} =
                discard
    ...
    test "OPTIONS endpoint":
        var ctx = Context.new(Request(HttpMethod.HttpOptions, "https://yumad.bro/"), Config())
        var app = App.new()
        app.options("/",
            proc (ctx: Context) {.async.} =
    ...
    test "PATCH endpoint":
        var ctx = Context.new(Request(HttpMethod.HttpPatch, "https://yumad.bro/"), Config())
        var app = App.new()
        app.patch("/",
            proc (ctx: Context) {.async.} =
                discard
    ...
    test "POST endpoint":
        var ctx = Context.new(PostRequest("https://yumad.bro/"), Config())
        var app = App.new()
        app.post("/",
            proc (ctx: Context) {.async.} =
    ...
    test "PUT endpoint":
        var ctx = Context.new(Request(HttpMethod.HttpPut, "https://yumad.bro/"), Config())
        var app = App.new()
        app.put("/",
            proc (ctx: Context) {.async.} =
                ctx.response.status(Http201)
    ...
    test "Can GET a endpoint already defined to handle POST requests":
        var ctx = Context.new(GetRequest("https://yumad.bro/memes"), Config())
        var app = App.new()
        app.post("/memes",
            proc (ctx: Context) {.async.} =
                ctx.response.status(Http201)
    ...
    test "Can POST a endpoint already defined to handle GET requests":
        var ctx = Context.new(PostRequest("https://yumad.bro/memes"), Config())
        var app = App.new()
        app.get("/memes",
            proc (ctx: Context) {.async.} =
                discard
    ...
    test "Can chain route definitions":
        ...
        ctx = Context.new(GetRequest("https://yumad.bro/memes"), Config())
        waitFor app.dispatch(ctx)
        check(ctx.response.getStatus() == Http200)
...
suite "Nested router":
    test "Can define a nested router":
        var ctx = Context.new(GetRequest("https://yumad.bro/api/v1/users"), Config())
        var app = App.new()
...
suite "Middlewares":
    ...
    test "Can register a middleware on a sub-router":
        var ctx = Context.new(GetRequest("https://yumad.bro/users/"), Config())
        var app = App.new()
...
suite "Error handling":
    ...
    test "Not found endpoint returns a 404 status code.":
        var ctx = Context.new(GetRequest("https://yumad.bro/an-undefined-url"), Config())
        var app = App.new()
        app.get("/",
            proc (ctx: Context) {.async.} =
                discard
